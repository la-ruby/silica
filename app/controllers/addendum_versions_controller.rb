...
class AddendumVersionsController < ApplicationController
  ...
  def update
    ...
    streams = clicked_streams(params[:what_was_clicked])
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: [
          ...
          turbo_stream.replace('apollo-underwriting-review-offer-pane',
                               partial: '/component/underwriting_review_offer_pane', locals: { project_id: project.id })
        ]
      end
    end
  end
  ...
  def clicked_streams(what_was_clicked)
    case what_was_clicked
    when 'send_addendum_version_to_seller'
      send_addendum_version_to_seller
    when 'sign_addendum_version'
      sign_addendum_version
    when 'discard_addendum_version'
      discard_addendum_version
    else
      saves
    end
  end
  ...
  def saves
    if params[:addendum_version].present?
      ...
      @addendum_version.update(expiration: begin
        DateTime.strptime(addendum_version_params[:expiration],
                          '%Y-%m-%dT%R').iso8601
      rescue StandardError
        nil
      end)
      @addendum_version.update(deadline: begin
        DateTime.strptime(addendum_version_params[:deadline],
                          '%Y-%m-%dT%R').iso8601
      rescue StandardError
        nil
      end)
    ...
    [turbo_stream.replace('flashes', partial: '/flashes', locals: { message: 'Saved' }),
     turbo_stream.replace(
       "addendum-#{@addendum_version.addendum.id}-addendum-version-#{@addendum_version.id}-panel",
       partial: '/component/addendum_version', locals: {
         addendum_version_id: @addendum_version.id, editable: true
       }
     )]
  end
  ...
  def sign_addendum_version
    [
      turbo_stream.replace('apollo-redirect',
                           partial: '/redirect', locals: { url:
                             @addendum_version.addendum.project
                             .create_docusign_envelope_for_addendum_version(@addendum_version.id) })
    ]
  end
  ...
  def discard_addendum_version
    ...
    [
      turbo_stream.replace('apollo-underwriting-review-offer-pane',
                           partial: '/component/underwriting_review_offer_pane',
                           locals: { project_id: @addendum_version.addendum.project.id }),
      turbo_stream.replace('flashes', partial: '/flashes',
                                      locals: { message: "Added draft #{new_addendum_version.version}" })
    ]
  end
end
