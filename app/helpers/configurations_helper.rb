...
module ConfigurationsHelper
  def configuration_face_tag(name, value, form)
    select_tag name.to_s,
               ...
               form: form,
               class: "form-select text-reset silica-weight-reset",
               data: {
  ...
  def configuration_color_tag(name, label, value, form)
    "#{label}: <input name=\"#{name}\" class=\"d-inline-block text-reset silica-weight-reset\" value=\"#{ j value }\" form=\"#{form}\" data-controller=\"submits-form-immediately\" data-action=\"submits-form-immediately#submit_form\" style=\"width: 100px\">".html_safe
  end
